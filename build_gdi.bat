...
if "%~nx1"=="" Goto Error1
if "%Game_Folder%"=="%Game_Folder:Extracted=%" Goto Error2
if not exist "%~nx1\bootsector\IP.BIN" Goto Error3
if not exist "%Game_Folder:~0,-10%\*.gdi" Goto Error4
CD %Game_Folder:~0,-10%
...
%GUI_Element_1%
Echo  Building new image please wait...
..\tools\buildgdi.exe -data "..\%~nx1" -ip "..\%~nx1\bootsector\IP.BIN" -output "" -gdi "%GDI%" -raw >NUL
Echo  Image creation complete.
timeout /t %Delay% >NUL
exit
